...
export const Sidebar = () => {
  ...
  return (
    <div className="px-4 flex flex-col gap-4">
      <h2 className="hidden 2xl:inline text-center mt-4 text-white text-2xl font-semibold">
        VyaparBook
      </h2>

      <div className="2xl:hidden mt-4 flex justify-center text-white items-center">
        <IoBookOutline size={30} />
      </div>

      <div>
        <div className="2xl:p-4 2xl:bg-[#205a51] rounded-xl select-none cursor-pointer flex gap-6">
      ...
      <div className="">
        <h2 className="hidden 2xl:inline uppercase ml-5 font-bold text-sm  text-[#439b8c]">
          Management
        </h2>

        <div className="mt-2 2xl:mt-5 text-lg flex flex-col gap-2">
          {menuItems?.map((item, index) => (
            <div key={index}>
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex gap-5 cursor-pointer font-medium p-2 2xl:px-4 2xl:py-2 rounded-md ${handleActiveLink(
                    {
                      isActive,
                    }
                  )}`
              ...
              >
                ...
                <p className="hidden 2xl:inline">{item.title}</p>
              </NavLink>
              {hoveredItem === index && (
                <span className="2xl:hidden relative -left-4 px-2 py-1 bg-gray-800 text-white text-xs rounded-md shadow-md">
                  {item.title}
                </span>
              )}
            </div>
          ...
          >
            ...
            <p className="hidden 2xl:inline">Log Out</p>
          </div>
        </div>
      </div>
      {showModal && (
